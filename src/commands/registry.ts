...
export function buildCommands(actions: EditorActions): Command[] {
  return [
    ...
    },
    {
      ...
      category: 'view',
      shortcut: 'Z',
      keywords: ['zen', 'focus', 'distraction', 'present', 'presentation', 'clean', 'hide', 'chrome', 'screenshot'],
      run: () => useUIStore.getState().toggleZenMode(),
    },
    {
      id: 'view.toggleViewMode',
      title: 'Toggle view mode (read-only)',
      category: 'view',
      shortcut: 'L',
      keywords: ['view', 'read-only', 'readonly', 'lock', 'locked', 'present', 'share'],
      run: () => actions.toggleEditingLock(),
    },
    {
      id: 'view.toggleGrid',
      title: 'Toggle grid',
      category: 'view',
      shortcut: 'G',
      keywords: ['grid', 'dots', 'guides', 'generation', 'lines', 'background'],
      run: () => useUIStore.getState().toggleShowGrid(),
    },
    {
